...
impl SetupParameters {
    pub fn run(&self, program_arguments: &ProgramParameters) -> ResultBtAny<()> {
        let script_content = ChangeTemplate::try_from(self)?.render()?;
        ...
        if program_arguments.dry {
            ...
            info!(
                "Creating all directories up until `{}`.",
                self.script_path.to_string_lossy()
            );
            File::create(&self.script_path)?.write_all(script_content.as_bytes())?;
            info!(
                "Wrote `{}` to `{}`.",
                script_content,
                self.script_path.to_string_lossy()
            ...
            println!(
                "Remember to add `. {}` to `.bashrc` or an equivalent.",
                self.script_path.to_string_lossy()
            );
        }
        Ok(())
...
impl TryFrom<&SetupParameters> for ChangeTemplate {
    ...
    fn try_from(value: &SetupParameters) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            to_binary: current_exe()?.to_string_lossy().into_owned(),
        })
    }
}
