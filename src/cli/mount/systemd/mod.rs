...
use crate::{
    cli::{ProgramParameters, mount::MountParameters},
    errors::ResultBtAny,
};
...
impl SystemdParamereters {
    pub fn run(
        &self,
        _program_arguments: &ProgramParameters,
        _mount_arguments: &MountParameters,
    ) -> ResultBtAny<()> {
...
impl ServiceTemplate {
    pub fn try_new(mount_path: &Path) -> ResultBtAny<Self> {
        Ok(ServiceTemplate {
            tfs_binary_path: canonicalize(current_exe()?)?.to_string_lossy().into_owned(),
            mount_path: canonicalize(mount_path)?.to_string_lossy().into_owned(),
