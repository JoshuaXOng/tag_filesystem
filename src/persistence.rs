...
use crate::{
    ...
    files::TfsFile,
    filesystem_capnp::tag_filesystem,
    inodes::{FileInode, TagInode},
...
#[builder]
pub fn new_root_fuser(
    uid: u32,
    gid: u32,
    permissions: u16,
    when_accessed: SystemTime,
    when_modified: SystemTime,
    when_changed: SystemTime,
    when_created: SystemTime,
) -> FileAttr {
    FileAttr {
        ino: FUSE_ROOT_ID,
        size: 0,
...
pub fn deserialize_tag_filesystem(read_location: impl BufRead) -> ResultBtAny<PersistedTfs> {
    let capnp_message = serialize_packed::read_message(read_location, ReaderOptions::new())?;
    let capnp_filesystem = capnp_message.get_root::<tag_filesystem::Reader>()?;

    let root_fuser = capnp_filesystem.get_root()?;
    let (when_accessed, when_modified, when_changed, when_created) = get_system_times!(root_fuser);
    return_errors!(
    ...
    for capnp_file in capnp_filesystem.get_files()? {
        let file_name = capnp_file
            .get_name()
            .map_err(AnyError::from)
            .and_then(|name| name.to_string().map_err(AnyError::from));
        ...
        return_errors!(
            "Not all file fields could be deserialized.",
            file_name,
            file_inode,
            when_accessed,
            when_modified,
    ...
    for capnp_tag in capnp_filesystem.get_tags()? {
        ...
        return_errors!(
            "Not all tag fields could be deserialized.",
            tag_name,
            tag_inode,
            when_accessed,
            when_modified,
...
pub fn serialize_tag_filesystem(
    write_location: impl Write,
    root_fuser: &FileAttr,
    tfs_files: Vec<&TfsFile>,
    tfs_tags: Vec<&TfsTag>,
) -> ResultBtAny<()> {
    type CapnpType = u32;
    ...
    return_errors!(
        "For root FUSE attributes not all fields could be serialized.",
        when_accessed,
        when_modified,
        when_changed,
        when_created
    ...
    let file_count = CapnpType::try_from(file_count).map_err(|e| {
        format!(
            "Cannot convert number of files `{}` to Cap'n Proto \
            length type. {e}",
            file_count
        )
    })?;
    ...
    for (file_index, tfs_file) in tfs_files.iter().enumerate() {
        ...
        return_errors!(
            format!(
                "For file with name `{}` and inode `{}`, not all \
            fields could be serialized.",
                tfs_file.name, tfs_file.inode
            ),
            when_accessed,
            when_modified,
            when_changed,
    ...
    let tag_count = CapnpType::try_from(tag_count).map_err(|e| {
        format!(
            "Cannot convert number of tags `{}` to Cap'n Proto \
            length type. {e}",
            file_count
        )
    })?;
    ...
    for (tag_index, tfs_tag) in tfs_tags.iter().enumerate() {
        ...
        return_errors!(
            format!(
                "For tag with name `{}` and inode `{}`, not all \
            fields could be serialized.",
                tfs_tag.name, tfs_tag.inode
            ),
            when_accessed,
            when_modified,
            when_changed
...
fn new_unix_epoch(seconds: u64, nanoseconds: u32) -> ResultBtAny<SystemTime> {
    UNIX_EPOCH
        ...
        .ok_or(
            format!(
                "Overflowed `SystemTime`, `{seconds}` seconds with \
