...
use crate::{errors::ResultBtAny_, inodes::FileInode, path_::get_configuration_directory, wrappers::PathExt};

pub trait TfsStorage {
    fn get_file_size(&self, file_inode: &FileInode) -> ResultBtAny_<u64>;
    fn get_last_accessed(&self, file_inode: &FileInode) -> ResultBtAny_<SystemTime>;
    fn get_last_modified(&self, file_inode: &FileInode) -> ResultBtAny_<SystemTime>;
    fn get_when_created(&self, file_inode: &FileInode) -> ResultBtAny_<SystemTime>;
    fn read(&self, file_inode: &FileInode, start_position: u64,
        read_amount: usize) -> ResultBtAny_<Vec<u8>>;
    fn write(&mut self, file_inode: &FileInode, start_position: u64,
        to_write: &[u8]) -> ResultBtAny_<()>;
    fn delete(&self, file_inode: &FileInode) -> ResultBtAny_<()>;
}
...
impl TfsStorage for StubStorage {
    fn get_file_size(&self, _file_inode: &FileInode) -> ResultBtAny_<u64> {
        Ok(0)
    }

    fn get_last_accessed(&self, _file_inode: &FileInode) -> ResultBtAny_<SystemTime> {
        Ok(SystemTime::UNIX_EPOCH)
    }

    fn get_last_modified(&self, _file_inode: &FileInode) -> ResultBtAny_<SystemTime> {
        Ok(SystemTime::UNIX_EPOCH)
    }

    fn get_when_created(&self, _file_inode: &FileInode) -> ResultBtAny_<SystemTime> {
        Ok(SystemTime::UNIX_EPOCH)
    }
    ...
    fn delete(&self, _file_inode: &FileInode) -> ResultBtAny_<()> {
        Ok(())
    }
}
