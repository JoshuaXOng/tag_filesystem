...
use crate::{errors::ResultBtAny_, inodes::{NamespaceInode, TagInodes, ANY_NAMESPACE_INODE},
    wrappers::write_iter};
...
impl IndexedNamepsaces {
    ...
    pub fn get_by_inode(&self, namespace_inode: &NamespaceInode) -> ResultBtAny_<&TfsNamespace> {
        self.namespaces.get(&namespace_inode)
            .ok_or(Self::get_namespace_404_message(namespace_inode).into())
    }

    pub fn get_by_inode_id(&self, inode_id: u64) -> ResultBtAny_<&TfsNamespace> {
    ...
    -> ResultBtAny_<NamespaceUpdate<'_>> {
        Ok(self.namespaces
