...
pub fn configure_tracing() -> (
    SubscriberBuilder<DefaultFields, Format, EnvFilter, fn() -> Stderr>,
    String,
) {
    let to_binary = current_exe().expect(SETUP_TRACING_EXPECTATION);
    let binary_name = to_binary
        .file_name()
        .expect(SETUP_TRACING_EXPECTATION)
        .to_string_lossy();
    let mut directive = format!("warn,{}=info,{}=info", env!("CARGO_PKG_NAME"), binary_name);
...
pub fn setup_normal_tracing<P: AsRef<Path>>(middle_scoping: P) {
    ...
    tracing_setup.0.with_writer(rotating_writer).init();
...
pub fn setup_syslog_tracing() -> ResultBtAny<()> {
    ...
    let syslog_writer = Syslog::new(CString::new(logger_name)?, syslog_options, syslog_facility)
        .ok_or(format!("Syslog for `{logger_name}` already exists."))?;
    tracing_setup.0.with_writer(syslog_writer).init();
    info!("Tracing directive is `{}`.", tracing_setup.1);
    Ok(())
}
...
fn checking_join_behaviour() {
    let mut path = PathBuf::from("/home").join("").join("jxo");
