...
use crate::{entries::TfsEntry, errors::ResultBtAny_, inodes::{FileInode, TagInodes}, unwrap_or,
...
impl IndexedFiles {
    ...
    pub fn get_by_inode_id(&self, inode_id: u64) -> ResultBtAny_<&TfsFile> {
    ...
    pub fn get_free_inode(&self) -> ResultBtAny_<FileInode> {
        let inodes_inuse = self.get_inuse_inodes();
        FileInode::try_from_free_inodes(inodes_inuse)
    }

    fn will_collide(&self, check_for: &TfsFile) -> ResultBtAny_<()> {
    ...
    -> ResultBtAny_<HashMap<FileInode, T>> {
    ...
    pub fn add(&mut self, to_add: TfsFile) -> ResultBtAny_<&TfsFile> {
...
impl<'a, 'b> FileUpdate<'a, 'b> {
    pub fn try_set_name(&mut self, name: String) -> ResultBtAny_<()> {
    ...
    pub fn try_set_inode(&mut self, inode: FileInode) -> ResultBtAny_<()> {
    ...
    pub fn try_set_tags(&mut self, tags: TagInodes) -> ResultBtAny_<()> {
    ...
    fn will_collide(&self) -> ResultBtAny_<()> {
