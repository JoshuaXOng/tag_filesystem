...
use crate::{
    ...
    entries::TfsEntry,
    errors::{ResultBt, StringExt},
    filesystem::TagFilesystem,
    inodes::{FileInode, NamespaceInode, TagInode, TagInodes, get_is_inode_root},
    storage::TfsStorage,
    tags::DEFAULT_TAG_PERMISSIONS,
    ttl::{ANY_TTL, NO_TTL},
};
...
impl<Storage: TfsStorage> Filesystem for TagFilesystem<Storage> {
    ...
    fn readdir(
        ...
        inode_id: u64,
        file_handle: u64,
        pagination_offset: i64,
        mut reply: ReplyDirectory,
    ) {
        match self.readdir_inner(
            request,
            inode_id,
            file_handle,
            pagination_offset,
    ...
    fn fsyncdir(
        ...
        match self.fsyncdir_inner(request, target_inode, file_handle, datasync) {
            Ok(message) => {
                reply.ok();
                info!(message);
            }
...
impl<Storage: TfsStorage> TagFilesystem<Storage> {
    fn create_inner(
        ...
        if !get_is_inode_root(parent_inode) && !NamespaceInode::get_is_namespace(parent_inode) {
            Err(ErrorReply::new(
                ENOENT,
                "Not child of TFS root nor a namespace.",
            ))?;
        }
        ...
        if get_is_inode_root(parent_inode) {
            ...
            let fuser_attributes = self
                .get_file_fuser(&file_inode)
        ...
        let namespace_inode = NamespaceInode::try_from(parent_inode)
            .map_err_inner(|e| ErrorReply::new(ENOENT, e.to_string()))?;
        let tfs_namespace = self
            .get_namespaces()
            .get_map()
            .get(&namespace_inode)
            .ok_or(ErrorReply::new(
                ENOENT,
                format!(
                    "Namespace with id \
    ...
    fn mkdir_inner(
        ...
        if !get_is_inode_root(parent_inode) {
            Err(ErrorReply::new(
                ENOENT,
                "Needs to be under the root directory.",
            ))?;
        }

        let is_file_conflicting = self
            .get_files()
        ...
        if is_file_conflicting {
            Err(ErrorReply::new(
                EINVAL,
                format!(
                    "Untagged file already exists with \
                name `{tag_name}`."
                ),
            ))?;
        }
    ...
    fn lookup_inner(
        ...
        if get_is_inode_root(parent_inode) {
            if get_is_a_namespace(&predicate) {
                ...
                return Ok(LookupReply {
                    ttl: NO_TTL,
                    attr: self
                        .get_namespace_fuser(&namespace_inode)
                        .map_err_inner(|e| ErrorReply::new(ENOENT, e.to_string()))?,
                    generation: ANY_GENERATION,
            ...
            let target_inode = self
                ...
                .ok_or(ErrorReply::new_with_level(
                    ENOENT,
                    Level::INFO,
                    format!(
            ...
            let fuser_attributes = self
                .get_fuser_attributes(target_inode)
                .map_err_inner(|e| ErrorReply::new(ENOENT, e.to_string()))?;
            return Ok(LookupReply {
                ttl: ANY_TTL,
                attr: fuser_attributes,
        ...
        if let Ok(parent_namespace) = self.get_namespaces().get_by_inode_id(parent_inode) {
            let fuser = self
                .get_inrange_tags(parent_namespace)
                .and_then(|tags| {
                    tags.into_iter().find(|tag| tag.name == predicate).ok_or(
                        format!(
                            "No matching neighbour tag w/ name `{}` \
                        for namespace `{}`.",
                            predicate, parent_namespace.inode
                        )
                        .into(),
                    )
                })
                ...
                .or(self
                    .get_file_by_name_and_namespace_inode(&predicate, &parent_namespace.inode)
    ...
    ) -> ResultBt<GetattrReply, ErrorReply> {
    ...
    fn readdir_inner(
        ...
        let pagination_offset_: usize = pagination_offset
            .try_into()
            .with_bt()
            .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Can't convert offset. {e}")))?;

        let add = |tfs_entry: &dyn TfsEntry, pagination_offset: i64, reply: &mut ReplyDirectory| {
            reply.add(
                tfs_entry.get_inode_id(),
                pagination_offset,
                tfs_entry.get_file_kind(),
                tfs_entry.get_name(),
            )
        };

        if is_listing_root {
            let mut tagless_files: Vec<_> =
                self.get_files().get_by_tags(&TagInodes::new()).collect();
            tagless_files.sort();
        ...
        let mut inrange_tags = self
            .get_inrange_tags(current_namespace)
            .map_err_inner(|e| {
                ErrorReply::new(
                    EINVAL,
                    format!(
        ...
        inrange_tags.sort();
        let inrange_tags = inrange_tags.into_iter().map(|tag| tag as &dyn TfsEntry);

        let mut inscope_files: Vec<_> = self
            .get_files_by_namespace_inode(&current_namespace.inode)
            .map_err_inner(|e| {
                ErrorReply::new(EINVAL, format!("Could not get files under namespace. {e}"))
            })?
            .collect();
        inscope_files.sort();
        let inscope_files = inscope_files.into_iter().map(|file| file as &dyn TfsEntry);

        for to_list in inscope_files.chain(inrange_tags).skip(pagination_offset_) {
            pagination_offset += 1;
    ...
    fn read_inner(
        ...
        let file_inode: FileInode = target_inode
            .try_into()
            .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Not a file inode. {e}")))?;
        let start_position: u64 = start_position.try_into().with_bt().map_err_inner(|e| {
            ErrorReply::new(EINVAL, format!("Offset value can't be converted. {e}"))
        })?;
        let read_amount: usize = read_amount.try_into().with_bt().map_err_inner(|e| {
            ErrorReply::new(EINVAL, format!("Amount to read can't be converted. {e}"))
        })?;

        let content_read = self
            .get_storage()
            .read(&file_inode, start_position, read_amount)
            .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Failed to read file. {e}")))?;
    ...
    fn fsyncdir_inner(
        ...
        if should_fsync_all {
            self.save_persistently().map_err_inner(|e| {
                ErrorReply::new(EINVAL, format!("Failed to save TFS state. {e}"))
            })?;
            return Ok("Saved all.");
        }
    ...
    fn rename_inner(
        ...
        if get_is_inode_root(previous_parent) && get_is_inode_root(new_parent) {
            self.rename_tag(&previous_name, new_name)
                .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Failed to rename tag. {e}")))?;
        ...
        if let (Ok(previous_parent), Ok(new_parent)) = (&_previous_parent, &_new_parent) {
            self.move_file(
                &previous_parent.tags.clone(),
                &previous_name,
                new_parent.tags.clone(),
                new_name,
            )
            .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Failed to rename file. {e}")))?;
            return Ok("Renamed file.");
        }

        let mut e = format!(
            "Renaming a tag, the parent has to be the root. \
            Renaming a file, the parent has to be a valid namespace. Previous \
            and new parent inodes are `{}` and `{}`.",
    ...
    fn write_inner(
        ...
        flags: i32,
        _lock_owner: Option<u64>,
    ) -> ResultBt<WriteReply, ErrorReply> {
        let byte_amount: u32 =
            to_write.len().try_into().with_bt().map_err_inner(|e| {
                ErrorReply::new(EINVAL, format!("Writing too much data. {e}"))
            })?;

        let file_inode: FileInode = target_inode
            .try_into()
            .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Is not a file inode. {e}")))?;
        let start_position: u64 = start_position
            .try_into()
            .with_bt()
            .map_err_inner(|e| ErrorReply::new(EINVAL, format!("Can't convert offset. {e}")))?;
    ...
    fn setattr_inner(
        ...
        let fuser_attributes = self.get_fuser_attributes(target_inode).map_err_inner(|e| {
            ErrorReply::new(ENOENT, format!("Inode does not match anything. {e}"))
        })?;
    ...
    fn rmdir_inner(
        ...
        self.delete_tag(&tag_name.to_string_lossy())
            .map_err_inner(|e| ErrorReply::new(ENOENT, format!("Failed to delete tag. {e}")))?;
