use crate::{
    ...
    inodes::{FileInode, TagInode, TagInodes},
    tags::TfsTag,
};
...
fn displaying_file_things() -> ResultBtAny<()> {
    ...
    let mut tfs_file = TfsFile::builder()
        ...
        .group(1001)
        .build();
    assert_eq!(format!("{}", &tfs_file as &dyn TfsEntry), "test_file(id=6)");

    assert_eq!(format!("{}", &tfs_file), "test_file(id=6, tags={})");
    tfs_file.tags.0.insert(4.try_into()?);
    assert_eq!(format!("{}", &tfs_file), "test_file(id=6, tags={ 4 })");
    tfs_file.tags.0.insert(7.try_into()?);
    assert_eq!(format!("{}", &tfs_file), "test_file(id=6, tags={ 4, 7 })");
...
fn displaying_tag_filesystem() -> ResultBtAny<()> {
    ...
    assert_eq!(
        format!("{}", tag_filesystem),
        "TagFilesystem(files=[], tags=[], namespaces=[])"
    );

    let file_1_inode = tag_filesystem
        .add_file()
        .file_name("file_1")
        .owner_id(1000)
        .group_id(1000)
    ...
    let tag_1_inode = tag_filesystem
        .add_tag()
        .tag_name("tag_1")
        .owner_id(1000)
        .group_id(1000)
    ...
    assert_eq!(
        format!("{}", tag_filesystem),
        format!(
            "TagFilesystem(files=[file_1(id={file_1_inode}, tags={{}})], \
            tags=[tag_1(id={tag_1_inode})], \
            namespaces=[])"
        )
    );

    let namespace_id = tag_filesystem
