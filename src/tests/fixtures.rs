...
pub fn with_tfs_mount(to_do: impl FnOnce(&PathBuf) -> ResultBtAny<()>) -> ResultBtAny<()> {
    ...
    let setup_payload = fuse_setup().expect(expectation);
    to_do(&setup_payload.0)?;
    fuse_cleanup(setup_payload).expect(expectation);
    Ok(())
}
...
fn fuse_setup() -> Result<FusePayload, Box<(dyn Error + 'static)>> {
    ...
    let temporary_directory = temporary_directory.path().to_path_buf();
...
fn fuse_cleanup((mount_directory, mount_handle): FusePayload) -> ResultBtAny<()> {
    info!(
        "Running cleanup for FUSE FS, unmounting at `{:?}`.",
        mount_directory
    );
    let umount_process = Command::new("umount").arg(mount_directory).status()?;
