...
use crate::{errors::{AnyError, ResultBtAny_}, filesystem::TagFilesystem};

pub fn with_tfs_mount(to_do: impl FnOnce(&PathBuf) -> ResultBtAny_<()>) -> ResultBtAny_<()> {
...
type FusePayload = (PathBuf, JoinHandle<ResultBtAny_<()>>);
...
fn fuse_setup() -> Result<FusePayload, Box<(dyn Error + 'static)>> {
    ...
    let temporary_directory_ = temporary_directory.clone();
    let mount_handle: JoinHandle<ResultBtAny_<()>> = thread::spawn(move || {
...
#[instrument]
fn fuse_cleanup((mount_directory, mount_handle): FusePayload) -> ResultBtAny_<()> {
